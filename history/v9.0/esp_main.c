 ...
 *
 * Purpose:
 *   This is the main entry point for the ESP32-S3 microcontroller.  The Pi
 *   sends serial (UART) commands containing steering angles and motor speeds;
 *   the ESP interprets them, drives two DC motors via an L298N H-bridge, and
...
/* Protocol constants — MUST match Raspberry Pi reference implementation */
#define PACKET_HEADER 0xA5  /* Start-of-frame marker (SOF). */
#define PACKET_FOOTER 0x5A  /* End-of-frame marker (EOF). */

/* LED GPIO assignments */
#define LED_GREEN_GPIO 2    /* Green LED GPIO. */
#define LED_RED_GPIO   4    /* Red LED GPIO. */
                ...
                if (!in_packet && byte == PACKET_HEADER) {
                    ...
                    if (byte == PACKET_FOOTER && packet_idx >= 8) {
                        ...
                        if (calc_crc == pkt->crc) {
                            ...
                            g_state.crc_errors++;
                            ESP_LOGW(TAG, "CRC error: calc=0x%04X pkt=0x%04X",
                                     calc_crc, pkt->crc);
                        }
                        in_packet = false;
                        packet_idx = 0;
...
static void timeout_monitor_task(void *arg) {
    ...
    while (1) {
        ...
        if (g_state.last_packet_us > 0 && elapsed > timeout_us
...
static void status_task(void *arg) {
    while (1) {
        g_state.uptime_ms += 100;
        vTaskDelay(pdMS_TO_TICKS(100));
    }
}

void app_main(void) {
    ESP_LOGI(TAG, "WRO 4WS Firmware v9.0");
    ...
    g_state.state = esp_selftest_all_passed(&g_state.selftest_result)
                    ? ESP_STATE_READY : ESP_STATE_ERROR;
    ...
    xTaskCreate(led_indicator_task, "led_ind", 1024, NULL, 5, NULL);
    xTaskCreate(status_task, "status", 1024, NULL, 3, NULL);
