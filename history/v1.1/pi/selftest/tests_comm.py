...
def register_comm_tests(runner: SelfTestRunner, uart):
    def test_packet_encode():
        ...
        data = pkt.encode()
        if len(data) < 8:
            return TestResult("packet_encode").failed(f"Packet too short: {len(data)} bytes")
        return TestResult("packet_encode").passed(f"{len(data)} bytes, CRC=0x{pkt.crc:04X}")
    ...
    def test_crc_detection():
        ...
        if decoded.decode(list(data)):
            return TestResult("crc_detection").failed("Should have detected corruption")
    ...
    def test_uart_esp_echo():
        ...
        return TestResult("uart_esp_echo").passed("Packet sent, awaiting ESP32 response")
    ...
    def test_esp_heartbeat():
        ...
        return TestResult("esp_heartbeat").failed("No ESP32 heartbeat - check connection")
